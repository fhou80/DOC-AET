...
def read_conll_file(data, path, aet_words):
    conll = {}
    ...
    with open(path, 'r', encoding='utf8') as f:
        ...
        for line in f:
            ...
            if line.startswith('-DOCSTART-'):
                ...
                conll[docname] = {'sentences': [], 'mentions': [], 'aet_words':[]}
                ...
                if line == '':
                    ...
                    # find the anonymous entity type words in sent
                    sentence = ' '.join(cur_sent)
                    for biw in aet_words['bigram']:
                        if sentence.find(biw) != -1 and biw not in cur_aets:
                            cur_aets.append(biw)
                            cur_doc['aet_words'].append(biw)
                    tokens = nltk.word_tokenize(sentence)
                    for tok in tokens:
                        tok = tok.lower()
                        tok = wnl.lemmatize(tok)
                        if tok in aet_words['unigram'] and tok not in cur_aets:
                            cur_aets.append(tok)
                            cur_doc['aet_words'].append(tok)
                    ##########################
                    cur_sent = []
...
def read_type_words(aet_path):
    aet_words = {'unigram': [], 'bigram': []}
    with open(aet_path, 'r') as aet:
        for line in aet:
            cont = line.strip().split('\t')[0]
            if cont.find(' ') > 0:
                aet_words['bigram'].append(cont)
            else:
                aet_words['unigram'].append(cont)
    return aet_words
...
class CoNLLDataset:
    ...
    def __init__(self, path, person_path, conll_path):
        print('load type words dictionary')
        self.aet_words = read_type_words(path + '/atee_type_dict_100d.txt')
        print('load csv')
        ...
        print('load conll')
        read_conll_file(self.train, conll_path + '/AIDA/aida_train.txt', self.aet_words)
        read_conll_file(self.testA, conll_path + '/AIDA/testa_testb_aggregate_original', self.aet_words)
        read_conll_file(self.testB, conll_path + '/AIDA/testa_testb_aggregate_original', self.aet_words)
        read_conll_file(self.ace2004, conll_path + '/wned-datasets/ace2004/ace2004.conll', self.aet_words)
        read_conll_file(self.aquaint, conll_path + '/wned-datasets/aquaint/aquaint.conll', self.aet_words)
        read_conll_file(self.msnbc, conll_path + '/wned-datasets/msnbc/msnbc.conll', self.aet_words)
        read_conll_file(self.clueweb, conll_path + '/wned-datasets/clueweb/clueweb.conll', self.aet_words)
        read_conll_file(self.wikipedia, conll_path + '/wned-datasets/wikipedia/wikipedia.conll', self.aet_words)
