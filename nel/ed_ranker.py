...
class EDRanker:
    ...
    def __init__(self, config):
        ...
        """ configure the typed entity embeddings"""
        config['aet_entity_embeddings'] = config['aet_entity_embeddings'] / \
                                      np.maximum(np.linalg.norm(config['aet_entity_embeddings'],
                                                                axis=1, keepdims=True), 1e-12)
        config['aet_entity_embeddings'][config['entity_voca'].unk_id] = 1e-10
        #===========================
    ...
    def prerank(self, dataset, predict=False):
        ...
        for content in dataset:
            ...
            if self.args.keep_ctx_ent > 0:
                ...
                aet_ctx_ids = [m['aet_ctx'] for m in content]
                aet_ctx_ids, aet_ctx_offsets = utils.flatten_list_of_lists(aet_ctx_ids)
                aet_ctx_ids = Variable(torch.LongTensor(aet_ctx_ids).cuda())
                aet_ctx_offsets = Variable(torch.LongTensor(aet_ctx_offsets).cuda())

                log_probs = self.prerank_model.forward(token_ids, token_offsets, entity_ids, aet_ctx_ids, aet_ctx_offsets, use_sum=True)
            ...
            for m in content:
                ...
                if true_pos >= len(named_cands):
                    if not predict:
                        ...
                        p_e_m[-1] = p
                        named_cands[-1] = m['gold'][0] # replace the last candidates with gold
                    else:
                        true_pos = -1
                ...
                if len(cands) == 0 and not predict:
                    continue
                elif len(cands) < self.args.n_cands_before_rank:  # padding? and mask the padding
                ...
                if conll_doc is not None:
                    ...
                    aet_ctx = [self.model.aet_word_voca.get_id(ae) for ae in aet_words]
    ...
    def train(self, org_train_dataset, org_dev_datasets, config):
        ...
        for e in range(config['n_epochs']):
            ...
            for dc, batch in enumerate(train_dataset):  # each document is a minibatch
                ...
                aet_ctx_ids = [m['aet_ctx'] for m in batch]
                aet_ctx_ids, aet_ctx_mask = utils.make_equal_len(aet_ctx_ids, self.model.aet_word_voca.unk_id)
                ...
                aet_ctx_ids = Variable(torch.LongTensor(aet_ctx_ids).cuda())
                aet_ctx_mask = Variable(torch.FloatTensor(aet_ctx_mask).cuda())

                scores = self.model.forward(token_ids, token_mask, entity_ids, entity_mask, aet_ctx_ids, aet_ctx_mask, p_e_m,
            ...
            if (e + 1) % eval_after_n_epochs == 0:
                ...
                for di, (dname, data) in enumerate(dev_datasets):
                    ...
                    if not dname.startswith('aida'):
                        avg_list.append(f1)
                    if dname == 'aida-A':
                    #if dname == 'aida-B':
                        dev_f1 = f1
                avg_f1 = np.average(avg_list)
                if avg_f1 > best_avg_f1:
                    print('save best average model to', self.args.model_path)
                    self.model.save(self.args.model_path+'_best_avg')
                    best_avg_f1 = avg_f1
                print("Average F1 = " + str(avg_f1) + ", Best Average F1 = " + str(best_avg_f1))
                ...
                if not_better_count == self.args.n_not_inc:
                    loss_save = np.array(loss_epoch)
                    np.save("/home/jeff/NLP_code/NED/NED/loss_ganea.npy", loss_save)
                    print("best average f1==" + str(best_avg_f1))
                    break
