...
class MulRelRanker(LocalCtxAttRanker):
    ...
    def __init__(self, config):
        ...
        # context and entity padding
        if self.use_pad_ent:
            self.pad_ent_emb = torch.nn.Parameter(torch.randn(1, self.emb_dims) * 0.1)
            self.pad_aet_ent = torch.nn.Parameter(torch.randn(1, self.aet_dims) * 0.1)
        ...
        # anonymous entities type
        self.aet_ent_ent_diag = torch.nn.Parameter(torch.randn(self.aet_dims) * 0.001)
        # relation embeddings, initialized with random values
    ...
    def forward(self, token_ids, tok_mask, entity_ids, entity_mask, aet_word_ids, aet_word_mask, p_e_m, gold=None):
        ...
        if self.use_local:
            local_ent_scores = super(MulRelRanker, self).forward(token_ids, tok_mask,
                                                                 entity_ids, entity_mask, aet_word_ids, aet_word_mask,
                                                                 p_e_m=None)
            ent_vecs = self._entity_vecs
            aet_ent_vec = self._aet_ent_vec
            #ent_vecs = self.typed_entity_embeddings(entity_ids)
        else:
            ent_vecs = self.entity_embeddings(entity_ids)
            local_ent_scores = Variable(torch.zeros(n_ments, n_cands).cuda(), requires_grad=False)

        # compute context vectors: averaging
        ...
        if self.use_pad_ent:
            ...
            aet_ent_vec = torch.cat([aet_ent_vec, self.pad_aet_ent.view(1, 1, -1).repeat(1, n_cands, 1)], dim=0)
        ...
        if n_ments == 1:
            ...
            # get the weights, i.e. the alpha value
            ...
            if self.mode == 'ment-norm':
                ...
                ent_ent_scores = torch.matmul(ent_rel_vecs,
                                 ...
                                 .permute(0, 2, 1, 3)
            #compute another Phi(ei, ej) using anonymous entities
            #aet_ent_ent_score = aet_ent_vec * self.aet_ent_ent_diag.view(1, 1, self.aet_dims)
            #aet_ent_ent_score = torch.matmul(aet_ent_ent_score.view(n_ments, 1, n_cands, -1),
            #                                 aet_ent_vec.view(1, n_ments, n_cands, -1).permute(0, 1, 3, 2)).permute(0, 2, 1, 3)
            # combine two Phi score
            #ent_ent_scores = ent_ent_scores + aet_ent_ent_score
        ...
        inputs = torch.cat([ent_scores.view(n_ments * n_cands, -1),
                            torch.log(p_e_m + 1e-20).view(n_ments * n_cands, -1)], dim=1)
        # get the combined score, then change view
        scores = self.score_combine(inputs).view(n_ments, n_cands)
