...
class NTEE(AbstractWordEntity):
    ...
    def forward(self, token_ids, token_offsets, entity_ids, aet_words, aet_offsets, use_sum=False):
        sent_vecs = self.compute_sent_vecs(token_ids, token_offsets, use_sum)
        entity_vecs = self.entity_embeddings(entity_ids)

        aet_ctx_vec = self.aet_word_embeddings(aet_words, aet_offsets)
        aet_entity_vecs = self.aet_entity_embeddings(entity_ids)
        aet_dims = aet_ctx_vec.size(1)
        ...
        aet_scores = torch.bmm(aet_entity_vecs, aet_ctx_vec.view(batchsize, aet_dims, 1)).view(batchsize, n_entities)
        # combine typed scores
        #scores = scores + aet_scores
        #scores = scores*0.8 + typed_scores*0.2
